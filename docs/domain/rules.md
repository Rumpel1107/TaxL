...
must respect, and a test case in the validation suite — **and every flag the sheet raises has been
resolved or accepted in writing.** Reading the source is not closing it, and neither is drafting the
sheet. Rule sheet examples are invented figures, never values taken from a real return (D31).

| Status | Meaning |
|---|---|
| ✅ | Covered by a rule sheet — its own or another rule's — and no flag open |
| 🟡 | Rule sheet written, one or more flags still open |
| ⬜ | Open — source not verified, no rule sheet, or blocked |

The rules at 🟡 have their sheet written; what is missing is resolving or accepting each flag listed
under *Open flags*. Rule sheets live in `docs/domain/rule-sheets/`.
...
| R18 | Fondo de Solidaridad Pensional as INCRNGO — closed inside R3's sheet, not duplicated here | E.T. art. 55; Ley 100 de 1993 arts. 20, 25–27; DIAN Oficio 909171 de 2021 | ✅ |
...
These are why a 🟡 is not a ✅. Each must be resolved or accepted in writing before its rule sheet
closes. The full statement of each flag lives in the `**Open:**` line of its own rule sheet; what
follows is the index, in rule order.

- **R3 — one INCRNGO item outside the registry.** State educational support (art. 46) is treated as
  INCRNGO by the engine and has no rule of its own.
- **R4 — one extension not covered, and it is in scope.** Art. 206 par. 5 extends the exemption to
  professional fees, and the MVP includes an employee with fees income opting for the benefit.
- **R5 — who qualifies is not specified.** Art. 387 par. 2 defines the dependent; the engine takes
  the count as an input it cannot verify.
- **R6 — one cap's ownership undecided.** Whether the 1.200 UVT mortgage interest cap belongs to
  the taxpayer or is shared when the loan has more than one debtor.
...
- **R10 — one date to confirm.** The TY2025 filing window ends 26-oct-2026 by business-day count,
  ...
  hard-coding.
- **R15 — one taxable moment not established.** The engine adds severance paid, severance under the
  traditional regime and severance consigned to a fund, and applies one exempt share to the total.
  Whether the taxable moment differs between the traditional regime and the fund is not established.
- **R16 — two monthly tests read annually.** The statute tests salary and allowance month by month;
  the engine divides salary by twelve and multiplies the cap by twelve. The two readings agree only
  when salary and allowance are even across the months.
